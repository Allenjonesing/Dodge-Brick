...
namespace BzKovSoft.RagdollTemplate.Scripts.Charachter
{
    ...
    public sealed class BzRagdoll : MonoBehaviour, IBzRagdoll
    {
        ...
        void FixedUpdate()
        {
            if (!isRagDollingFromBeingShot && _state == RagdollState.WaitStablePosition &&
        ...
        public void BrickCollisionDetected()
        {
            BrickCollisionDetected(false);
        }

        public void BrickCollisionDetected(bool isHeadHit)
        {
            UnityEngine.Debug.Log(isHeadHit);
